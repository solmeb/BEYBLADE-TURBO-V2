...
const SESSION_FILE = process.env.SESSION_FILE || './auth_info_multi.json';
  ...
  sock.ev.on('messages.upsert', async m => {
    try {
      ...
      if (command.ownerOnly) {
        ...
        if (!ownerJid) {
          await sock.sendMessage(jid, { text: 'OWNER_ID non défini dans .env : impossible de vérifier les permissions owner.' }, { quoted: msg });
          return;
        }
        const sender = msg.key.participant || msg.key.remoteJid; // group or private
        if (sender !== ownerJid && msg.key.remoteJid !== ownerJid) {
